...
if (isset($_POST['valider'])) {
    ...
    if (!empty ($_POST['categorie']) && !empty ($_POST['prix']) && !empty ($_POST['nom']) && !empty ($_POST['desc'])//les champs partenaire et partenaire web ne sont pas obligatoire, tout comme l'image
        ...
        $DB->insert("UPDATE produits SET nom_produit = '$nom', prix_produit = '$prix', description_produit = '$description', categorie_produit = '$categorie' , partenaire_produit = '$partenaire' , partenaireweb_produit = '$part_web', quantite_produit = '$quantite', resultatTest = '$resultatTest', produit_actif = '$produitActif' WHERE id_produit = '$id' ");//insertion en BDD
